...
# kinds and is orchestrated by the matching runner below. "body" and "mtp"
# are the active paths; "visual" is acknowledged-but-skipped (text-only
# staging strips the visual blocks from the model graph, so no Linears
# would match even if we tried — visual probing is a separate project).
# ---------------------------------------------------------------------------
...
def _build_streaming_context(model_path: str, *,
                             ...
                             offload_folder: str,
                             cache_headroom_gb: float = 75.0) -> StreamingContext:
    ...
    t0 = time.time()
    print(f"[incremental] base_prefix={base_prefix!r}  layers={num_layers}  "
    ...
    print(f"[incremental] model ready in {time.time()-t0:.1f}s", flush=True)

    print(f"[incremental] building install resolvers for {num_layers} layers ...",
          flush=True)
    t_res = time.time()
    install_resolvers = [
    ...
    print(f"[incremental] resolvers built: "
    ...
    print(f"[incremental] layer cache budget={cache_bytes/(1024**3):.1f} GB "
