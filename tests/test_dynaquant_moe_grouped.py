...
from quantization.dynaquant_pkg.dynaquant import dynaquant_moe
...
def _make_layer(grouped_bits: bool = True):
    ...
    dynaquant_moe._build_expert_groups(layer)
    return layer
...
class TestDynaquantMoEGrouped(unittest.TestCase):
    ...
    def test_grouped_eager_matches_legacy(self):
        ...
        method = dynaquant_moe.DynaQuantFusedMoEMethod.__new__(
            dynaquant_moe.DynaQuantFusedMoEMethod
        )
        original = dynaquant_moe._get_fused_linear
        dynaquant_moe._get_fused_linear = lambda: _fake_fused_linear
        try:
            x = torch.tensor(
                [
                    [1.0, 2.0, 3.0, 4.0],
            ...
            dynaquant_moe._get_fused_linear = original
