...
from quantization.dynaquant_pkg.dynaquant import dynaquant_moe as prismaquant_moe
...
def _make_layer(grouped_bits: bool = True):
    layer = _DummyLayer()
    layer.dynaquant_num_experts = 3
    layer.dynaquant_hidden_size = 4
    layer.dynaquant_intermediate_size = 3
    layer.dynaquant_group_size_h = 2
    layer.dynaquant_group_size_i = 1
    device = "cpu"
    ...
    layer.dynaquant_w13_table = [
        (0, 2, 2, 2, 4, 4, 3, 4),
        (4, 2, 6, 2, 4, 4, 3, 4),
        (8, 2, 10, 2, e2_bits, e2_bits, 3, 4),
    ]
    layer.dynaquant_w2_table = [
        (0, 2, 4, 4, 3),
        (2, 2, 4, 4, 3),
        (4, 2, e2_bits, 4, 3),
    ]
...
def _legacy_apply(layer, x, topk_weights, topk_ids):
    x_f32 = x.float()
    out = torch.zeros(x.shape[0], layer.dynaquant_hidden_size, dtype=x.dtype)
    for eidx in range(layer.dynaquant_num_experts):
        ...
        g_off, g_sz, u_off, u_sz, g_bits, u_bits, out_f, in_f = layer.dynaquant_w13_table[eidx]
        gate = _fake_fused_linear(
            ...
            out_f,
            in_f,
            group_size=layer.dynaquant_group_size_h,
        )
        ...
        down = _fake_fused_linear(
            ...
            d_out,
            d_in,
            group_size=layer.dynaquant_group_size_i,
        )
...
class TestDynaquantMoEGrouped(unittest.TestCase):
    def test_build_expert_groups_buckets_by_quant_signature(self):
        ...
        groups = layer.dynaquant_expert_groups
    ...
    def test_grouped_eager_matches_legacy(self):
        ...
        method = prismaquant_moe.DynaQuantFusedMoEMethod.__new__(
            prismaquant_moe.DynaQuantFusedMoEMethod
        )
