# Qwen3.6 PrismaQuant State - 2026-04-19

## Summary

The repo now has a working end-to-end path from PrismaQuant analysis to a locally served
...
### 1. PrismaQuant analysis on Qwen3.6 MoE

The Qwen3.6 MoE path was fixed so that PrismaQuant correctly sees:
...
The working route instead uses the repo’s PrismaQuant export/runtime path:

- `quantization/export_prismaquant_ct.py`
- `quantization/prismaquant_pkg/patch_vllm.py`
- `quantization/prismaquant_pkg/prismaquant_moe.py`
...
The current PrismaQuant MoE runtime path is a local proof path, not an upstream-friendly
vLLM architecture.
...
But packed Qwen3.6 MoE currently required the repo’s PrismaQuant-specific runtime bridge
...
4. Decide whether to spend effort on:
   - making the PrismaQuant custom MoE path faster locally
   - or translating the winning PrismaQuant policy into a more upstream-compatible format/runtime path

## Files To Know

- `quantization/prismaquant/allocator.py`
- `quantization/prismaquant/measure_quant_cost.py`
- `quantization/export_prismaquant_ct.py`
- `quantization/prismaquant_pkg/patch_vllm.py`
- `quantization/prismaquant_pkg/prismaquant_moe.py`
- `recipes/qwen3.5-35b-a3b-prismaquant.yaml`

## Bottom Line

The project crossed the important threshold:

- PrismaQuant analysis works on Qwen3.6 MoE
