...
# Install deps
RUN --mount=type=cache,id=uv-cache,target=/root/.cache/uv \
     uv pip install "torch==2.12.0.dev20260405+cu130" torchvision torchaudio triton --index-url https://download.pytorch.org/whl/nightly/cu130 && \
...
# DynaQuant: install kernel package and patch vLLM for arbitrary bit-width quantization
COPY quantization/dynaquant_pkg/dynaquant /usr/local/lib/python3.12/dist-packages/dynaquant
COPY quantization/dynaquant_pkg/patch_vllm.py /tmp/patch_vllm.py
