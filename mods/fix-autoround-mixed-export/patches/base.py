...
# AUTO_ROUND_FUSED_PROMOTE_V1 helper -------------------------------------
def _ar_precision_rank(cfg: dict) -> tuple:
    """Higher tuple = higher precision. Used to pick the "best" sibling."""
    bits = cfg.get("bits", 16) or 16
    act_bits = cfg.get("act_bits", bits) or bits
    # Prefer higher weight bits, then higher activation bits. Ties broken
    # by bigger group_size (mxfp8 group=32 vs nvfp4 group=16 means mxfp8
    # wins at the same bit count, which is what we want since mxfp8 is
    # the higher-precision format at 8-bit).
    return (bits, act_bits, cfg.get("group_size", 0) or 0)
...
def _ar_promote_fused_schemes(layer_config: dict) -> dict:
    """Force fused-projection siblings to share the highest-precision scheme."""
    # Collect siblings by (parent_prefix, container, member_tuple)
    sibling_sets: dict[tuple, dict[str, str]] = {}
    for name in list(layer_config.keys()):
        for container, members_list in _AR_FUSED_GROUPS.items():
            token = f".{container}."
            ...
            for members in members_list:
                if suffix in members:
                    key = (prefix, container, members)
                    sibling_sets.setdefault(key, {})[suffix] = name
                    break
    for (prefix, container, members), present in sibling_sets.items():
        if len(present) < 2:
            continue  # nothing to reconcile
        # Find the highest-precision scheme among siblings
        best_name = max(present.values(), key=lambda n: _ar_precision_rank(layer_config[n]))
        best_cfg = layer_config[best_name]
        # Preserve per-layer fields that shouldn't be cross-copied
        protect_keys = {"scale_dtype", "fixed_by_user"}
        for sibling_name in present.values():
            if sibling_name == best_name:
                continue
            existing = layer_config[sibling_name]
            new_cfg = dict(existing)
            for k, v in best_cfg.items():
                if k in protect_keys:
                    continue
                new_cfg[k] = v
            layer_config[sibling_name] = new_cfg
    return layer_config
...
class BaseCompressor(object):
    ...
    def _gen_auto_scheme(self) -> dict[str, dict]:
        ...
        # AUTO_ROUND_FUSED_PROMOTE_V1: vLLM fuses q/k/v -> qkv_proj and
        # gate/up -> gate_up_proj during weight loading. Different schemes
        # across siblings would produce incompatible fused tensor layouts.
        # Promote each sibling set to the highest-precision member.
        layer_config = _ar_promote_fused_schemes(layer_config)
        return layer_config
