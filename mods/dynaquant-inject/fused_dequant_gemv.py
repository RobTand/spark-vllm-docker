...
Supports bit widths 3-8 (the practical range for DynaQuant).
Bit widths 4 and 8 should prefer native NVFP4/FP8 paths when available;
this kernel targets the 3, 5, 6, 7 bit sweet spot.
...
def _fused_dequant_gemv_kernel(
    ...
    for k_start in range(0, K, BLOCK_K):
        ...
        # Load bytes — need up to 2 bytes per value for cross-byte extraction
        # Guard b1 read: only needed when bits cross a byte boundary
...
def _fused_dequant_gemm_kernel(
    ...
    for k_start in range(0, K, BLOCK_K):
        ...
        w_tile = q_signed * s  # (BLOCK_M, BLOCK_K) dequantized weights
